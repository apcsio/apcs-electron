...
/**
 * @description Views are the visual building blocks of an application. They allow for
...
module.exports = function(data) {
	 ...
	 * @return {(str|View)} - Returns type of view or view itself.
	 */
	 ...
	 * @param {View} newView - View to be added to parent view
	 ...
	 * @param {View} newView - id to be given to added child view.
	 * @param {str} [before] - id of view which newView should be added before
	 */
	 ...
	 * @param  {(View|str)} child - Child view/id to be moved.
	 * @param  {(View|str)} before - Child view/id which target is moved before.
	 */
	 ...
	 * @return {View} - Returns newly sorted view.
	 */
	...
	/**
	 * Returns whether the view is empty (has no children).
	 * @return {Boolean} - Returns boolean describing emptiness of View.
	 */
	 ...
	 *		and view.can.getValue, so both must be manually set in order for this function to work
	 *		as expected.
	 * @param  {function} set - Function for setting the value of this View
	 * @param  {function} get - Function for getting the value of this View
	 * @return {View} - Returns this View.
	 */
	 ...
	 * @return {View} - Returns parent view with given id or direct parent.
	 */
	 ...
	 */
 	 ...
 	 * Returns whether or not this view has a parent of the given type.
 	 * @return {Boolean} - whether or not this view has a parent of the given type
 	 */
	view.hasParent = function(parentType) {
		if (Is.str(parentType)) {
			var p = view.is.childOf;
			while (Is.def(p) && p.is.a != parentType) {
				p = p.is.childOf;
			}
			return Is.def(p);
		}
	 ...
	 * @return {View} - Returns sibling View
	 */
	 ...
	 * @return {View} Returns previous sibling of current view.
	 */
	 ...
	 * @return {View} - Returns next sibling of current view.
	 */
	view.next = function() {
		if (view.hasParent()) {
			var index = view.parent().is.ordering.indexOf(view.getId());
	 ...
	 * @return {View} - Returns last child of this view.
	 */
	view.last = function() {
	...
	view.childAt = function(index) {
		if (index < view.is.ordering.length)
			return view.child(view.is.ordering[index]);
		else return View();
	};
	...
	/**
	 * Returns an identical copy of this element.
	 * @return {View} - Returns an identical copy of this element.
	 */
	view.copy = function() {
		var copy = View(type, {
			tag: view.is.tagged,
	...
	view.isSelecting = function(select) {
		return select == view.is.selecting;
	};
	...
	view.bm = function(b, m, c) {
		if (Is.undef(m) && view.is.bm[b])
			...
			if (view.is.bm[b])
				view.bem(b, null, view.is.bm[b], false);
			view.is.bm[b] = m;
		}
	...
	view.css = function(path) {
		if (! window._view_css)
			window._view_css = {};

		if (! path.indexOf('.css') == path.length - 4)
			path += '.css';
		if (Is.undef(window._view_css[path])) {
			window._view_css[path] = document.createElement('link');
			window._view_css[path].type = 'text/css';
			window._view_css[path].rel = 'stylesheet';
			window._view_css[path].href = '/css/' + path;
			document.getElementsByTagName("head")[0].appendChild(window._view_css[path]);
		}
		return view;
	};
	...
	/**
	 * removes the given state from this view.
	 * @param  {View}  t - View to check given state.
	...
	/**
	 * Returns whether this view is not set to the given state.
	 * @param  {View}  t - View to check given state status.
	...
	view.scroll = function(to, duration) {
		...
		if (Is.num(to) && ! view.is.scrolling) {
			...
			if (duration == 0) {
				...
				var scrollTimeout,
					endScroll = function(stop) {
						if (stop === true) view.$.stop();
						view.is.scrolling = false;
						view.$.off("scroll mousedown wheel DOMMouseScroll mousewheel keyup touchmove");
					};

				view.$.on("scroll mousedown wheel DOMMouseScroll mousewheel keyup touchmove", function() {
			       endScroll(true);
				...
				scrollTimeout = setTimeout(endScroll, duration || 400);
			}
		}
	};
	...
	view.offset = function(direction) {
		if (direction == 'left') {
			...
			return view.element.offsetWidth;
		}
		else if (direction == 'height') {
			return view.element.offsetHeight;
		}
		else return 0;
	};
